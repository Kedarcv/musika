class RestaurantDashboard {
    constructor() {
        ...
        this.initializeAvailableRestaurants(); // New function to initialize available restaurants
    ...
    async initializeAvailableRestaurants() {
        try {
            const response = await fetch('/data/restaurants.json'); // Fetching the restaurant data
            const restaurants = await response.json();
            
            const container = document.getElementById('availableRestaurants');
            container.innerHTML = restaurants.map(restaurant => this.createRestaurantCard(restaurant)).join('');
        } catch (error) {
